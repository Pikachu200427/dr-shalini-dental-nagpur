...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function checkAuth() {
    const isAuthenticated = sessionStorage.getItem('dr_shalini_admin_auth');
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
      ...
      .then(result => {
        if (result.success) {
          sessionStorage.setItem('dr_shalini_admin_auth', 'true');
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', () => {
      sessionStorage.removeItem('dr_shalini_admin_auth');
      checkAuth();
    });
  }
  ...
  function initDashboard() {
    ...
    navButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (tab === 'settings') loadClinicSettings();
        if (tab === 'reviews') loadReviewsManager();
      });
    });
      ...
      fetch('/api/dr_shalini/bookings')
        ...
        .then(bookings => {
          ...
          if (recentList) {
            ...
            const sortedRecent = bookings.slice(0, 5); // Already ordered descending by server
            
            if (sortedRecent.length === 0) {
              recentList.innerHTML = '<tr><td colspan="6" style="text-align: center; color: var(--text-light); padding: 15px;">No appointments recorded yet.</td></tr>';
              return;
            }
    ...
    if (filterSelect) {
      // Recreate to clear old event listeners
      ...
      fetch('/api/dr_shalini/bookings')
        ...
        .then(bookings => {
          ...
          if (filteredBookings.length === 0) {
            allList.innerHTML = `<tr><td colspan="7" style="text-align: center; color: var(--text-light); padding: 30px;">No appointments found matching this status.</td></tr>`;
            return;
          }
    ...
    function attachTableButtonListeners(tbody, reloadCallback) {
      ...
      tbody.querySelectorAll('.action-btn.delete').forEach(btn => {
        btn.addEventListener('click', () => {
          if (confirm('Are you sure you want to delete this appointment booking record?')) {
            const bookingId = btn.getAttribute('data-id');
    ...
    function updateBookingStatus(id, newStatus, callback) {
      fetch('/api/dr_shalini/bookings/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    function deleteBooking(id, callback) {
      fetch('/api/dr_shalini/bookings/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (settingsForm) {
      // Recreate to clear old submit listeners
      ...
      newSettingsForm.addEventListener('submit', (e) => {
        ...
        fetch('/api/dr_shalini/settings', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ phone, address, hours })
        })
        .then(res => res.json())
        .then(result => {
          if (result.success) {
    ...
    if (changePasswordForm) {
      ...
      newPasswordForm.addEventListener('submit', (e) => {
        ...
        fetch('/api/dr_shalini/change-password', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    function loadReviewsManager() {
      fetch('/api/dr_shalini/reviews')
        ...
        .then(reviews => {
          ...
          if (reviews.length === 0) {
            adminReviewsList.innerHTML = '<div style="color: var(--text-light); text-align: center; padding: 20px;">No patient reviews stored in the database.</div>';
            return;
          }
          ...
          // Attach Delete Review Listeners
          adminReviewsList.querySelectorAll('.delete-review-btn').forEach(btn => {
            btn.addEventListener('click', () => {
              if (confirm('Are you sure you want to remove this patient testimonial?')) {
    ...
    if (addReviewForm) {
      // Recreate to clear old listeners
    ...
    function deleteReview(id) {
      fetch('/api/dr_shalini/reviews/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id })
      })
      .then(res => res.json())
      .then(result => {
        if (result.success) loadReviewsManager();
      })
